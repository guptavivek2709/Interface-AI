...
function errorStatus(error: unknown): number {
  ...
  if (error instanceof RunManagerError) {
    ...
    if (error.code === "RUN_NOT_APPROVABLE" || error.code === "RUN_NOT_CANCELLABLE") return 409;
...
function projectIncidents(value: unknown): unknown[] {
  ...
  return value.slice(0, 500).flatMap((entry) => {
    ...
    const category = source.category === "recoverable" || source.category === "failure" || source.category === "escalation"
      ? source.category
      : "failure";
    return [{
      code,
...
function publicSnapshot(
  ...
  if (inputEntry?.artifact.schemaVersion === "2.0") {
  ...
  if (result) {
    ...
    if (result.status === "success" && result.outputs && typeof result.outputs === "object") {
      ...
      if (entry?.artifact.schemaVersion === "2.0") {
  ...
  if (Array.isArray(summary)) {
    ...
    const authorized =
      ...
      (principal.roles.includes(session.principal.role) ||
        (session.principal.role === "teller" && principal.roles.includes("supervisor")));
...
export function buildApiServer(options: ApiServerOptions): FastifyInstance {
  ...
  app.post("/api/v1/auth/logout", async (request, reply) => {
    ...
    try {
      ...
      if (ownedActiveRuns.some((run) => run.phase === "running" || run.phase === "recovering")) {
        return reply.status(409).send({
          error: {
  ...
  app.get("/api/v1/capabilities", async () => ({ capabilities: options.catalog.list() }));
  app.get<{ Params: { id: string; version: string } }>(
    ...
    async (request, reply) => {
      ...
      return { capability: entry.metadata, artifact: entry.artifact };
    },
  );
  ...
  app.post("/api/v1/runs", async (request, reply) => {
    ...
    if (
      ...
      return reply.status(403).send({
        error: {
          ...
          message:
            "Sign out, authenticate as a supervisor, establish a supervisor MERIDIAN session, and start this capability again.",
        },
      });
    }
    const entry = options.catalog.resolve(capability.id, capability.version);
    if (!entry || entry.artifact.schemaVersion !== "2.0") {
      return reply.status(409).send({
    ...
    const inputs = normalizeInputs(artifact.inputs, body.inputs as Record<string, RunValueV2>);
  ...
  app.get("/api/v1/runs", async (request) => {
    ...
    return {
      runs: live
        .filter((run) => runOwners.get(run.runId) === principal.subject)
  ...
  app.get<{ Params: { runId: string } }>("/api/v1/runs/:runId/events", async (request, reply) => {
    ...
    const unsubscribe = options.runs.subscribe(
      request.params.runId,
      (event) => writeSse(reply, event, options.sessions, options.catalog, principal),
      { afterEventId },
    );
    let closed = false;
  ...
  app.post<{ Params: { runId: string } }>("/api/v1/runs/:runId/approve", async (request, reply) => {
    ...
    const effectiveRoles = principal.roles.filter((role) =>
      session.principal?.role === "supervisor" ? true : role === "teller",
    );
  ...
  app.post("/api/v1/chat", async (request, reply) => {
    ...
    try {
      ...
      const metadata = {
        ...
        fallbackFrom: route.metadata.fallbackFrom,
      };
      if (route.kind === "reply") {
...
function openApiDocument() {
  return {
    ...
    components: {
      ...
      schemas: {
        ...
        RunRequest: {
          ...
          required: ["capabilityId", "artifactDigest", "inputs"],
          properties: {
    ...
    paths: {
      ...
      "/api/v1/runs": {
        get: { summary: "List only the authenticated operator's retained runs" },
        post: {
      ...
      "/api/v1/chat": {
        post: { summary: "Return an Anthropic or deterministic capability proposal; never starts or approves a run" },
      },
    },
  };
}
